				...
				for (int y = -1; y <= 1; y++) {   //8个方向扫描 来回扫2次（见下方代码）
					for (int x = -1; x <= 1; x++) {
						if (x == 0 && y == 0) continue;
							...
							case 0: break;// 无效
							case 4:ScoreMap[row][column] += 5000; break;//活2
							case 6:ScoreMap[row][column] += 13500; break;//连活3，但只有1方向可以正常冲4  eg001X102
							case 7:ScoreMap[row][column] += 18000; break;//冲4 
							case 8:ScoreMap[row][column] += 16200; break;//连活3
							case 9:ScoreMap[row][column] += 7000000; break;//活4
							case 10:ScoreMap[row][column] += 80000000; break;//黑子走完必胜
							default:break;
							}
							BlackNum = 0;
							FrontEmptyNum = 0;
							...
							for (e = -4; e <= 4; e++) {
								...
								if (e == 0)continue;                  //前面有else控制【row】【line】处是空格
									...
									if (Bounday_judgement) {
										...
										if (e < -2) {//前面只剩-4、-3，易写 00 01 10 11  （-2到0对应101才有效）
											if (BoardMap[checkingrow][checkingcolumn] == 1) black_front_jumpNum1++;
											if (BoardMap[checkingrow][checkingcolumn] == 0) {
												black_front_emptyNum1++;  //10101用处较小
												black_front_jumpNum1 = 0;
											}
											if (BoardMap[checkingrow][checkingcolumn] == 2) {
												black_front_jumpNum1 = 0;
												black_front_emptyNum1 = 0; //不能break
											}
										}
										...
										if (e > 0) {
											if (BoardMap[checkingrow][checkingcolumn] == 2) {  //空点周围非空则落子后构不成跳形
												...
												black_behind_emptyNum1++;// state=3：black_behind_jumpNum不再改变 
												state = 3;   //若后面全是空格，则black_behind_emptyNum1正常加
											}
							...
							case 0: break;//无进攻能力
							case 3:ScoreMap[row][column] += 5000; break;//跳活2（2500*2）
							case 5:ScoreMap[row][column] += 22000; break;//冲连（11000*2）
							case 6:ScoreMap[row][column] += 18000; break;//跳活三（9000*2）  //受打分方式影响
							...
							for (e = 1; e <= 4; e++) {
								...
								if (Bounday_judgement) { 
									if (BoardMap[checkingrow][checkingcolumn] != 0)break;//1X后1/2都不是被动跳形
							...
							case 6:ScoreMap[row][column] += 18000; break;//跳活三
							default:break;
							}/*统计黑子得分*/
							black_front_jumpNum2 = 0;
							black_behind_jumpNum2 = 0;
							...
							case -1:ScoreMap[row][column] += 5; break;//开辟新区域
							case 0: break;                            //没有进攻能力的点不得分
							case 4:ScoreMap[row][column] += 9000; break;   //活2
							case 6:ScoreMap[row][column] += 14000; break;  //连活3，但只有1方向可以正常冲4 
							case 7:ScoreMap[row][column] += 24000; break;  //等价于冲4  eg 102X2210
							case 8:ScoreMap[row][column] += 19500; break;  //连活3
							case 9:ScoreMap[row][column] += 8000000; break;//活4
							case 10:ScoreMap[row][column] += 100000000; break;//连五：走完白子必胜
							default: break;
							}
							WhiteNum = 0;
							...
							for (e = -4; e <= 4; e++) {
								...
								if (e == 0)continue;                  //前面有else控制【row】【line】处是空格
									...
									if (Bounday_judgement) {
										...
										if (e < -2) {//前面只剩e=-4、-3，情况只剩00 01 10 11  （-2到0对应10X才有效）
											if (BoardMap[checkingrow][checkingcolumn] == 2) white_front_jumpNum1++;
											if (BoardMap[checkingrow][checkingcolumn] == 0) {
							...
							case 5:ScoreMap[row][column] += 25000; break;//冲连（12500*2）
							...
							case 0:break;
							case 3:ScoreMap[row][column] += 50; break;
